...
<html>
    ...
    <body onload="getData();">
        ...
        <div style="display: none;" id="dialog_add_client" title="Mantenimiento Inventario">
                    ...
                    <div>
                        <select name="txtalmacenid" id="txtalmacenid" class="search" data-placeholder="Seleccione almacen">
                            <c:forEach var="lisAlmacen" items="${listaAlmacen}">
                                <option value="<c:out value="${lisAlmacen.almacenid}"></c:out>"><c:out value="${lisAlmacen.denominacion}"></c:out></option>
                            </c:forEach>
                        </select>
                    </div>
                </div>     
                    ...
                    <div>
                        <select name="txtproductoid" id="txtproductoid" class="search" data-placeholder="Seleccione producto">
                            <c:forEach var="lisProducto" items="${listaProducto}">
                                <option value="<c:out value="${lisProducto.productoid}"></c:out>"><c:out value="${lisProducto.descripcion}"></c:out></option>
                            </c:forEach>
                        </select>
                    </div>
                </div>  
        ...
        <div role="main" id="main" class="container_12 clearfix">
            <section id="content" class="container_12 clearfix" data-sort="true">
                <form action="" class="grid_12">
                    ...
                    <legend>LISTA INVENTARIO</legend>   
                    <div class="content">
                        <div class="tabletools">
                            <div class="left">
                                <a class="open-add-client-dialog" href="javascript:void(0);"><i class="icon-plus"></i>Nuevo Inventario</a>
                            </div>                         
                        </div>
                        <table id="list"></table> 
                        <div id="page"></div>
        ...
        <script>
            function getData() {
                $(document).ready(function() {
                    $("#list").jqGrid({
                        url: "recuperarInventario.htm",
                        datatype: 'json',
                        height: 'auto',
                        width: 'auto',
                        colNames: ['NRO', 'ALMACEN', 'COD. PROD.', 'UND. MEDIDAD', 'TIPO DOC.', 'TIPO MOV.', 'FECHA MOV.', 'CANT.', 'P. UNIT', 'P. TOTAL', 'ESTADO'],
                        colModel: [
                            {name: 'inventarioid', index: 'inventarioid', width: 40, sortable: false, align: 'center'},
                            {name: 'denominacionA', index: 'denominacionA', width: 150, sortable: false, align: 'left'},
                            {name: 'codigoproducto', index: 'codigoproducto', width: 80, sortable: false, align: 'center'},
                            {name: 'denominacionUM', index: 'denominacionUM', width: 150, sortable: false, align: 'left'},
                            {name: 'descripcionTD', index: 'descripcionTD', width: 150, sortable: false, align: 'left'},
                            {name: 'denominacionTM', index: 'denominacionTM', width: 120, sortable: false, align: 'left'},
                            {name: 'fechamovimiento', index: 'fechamovimiento', width: 80, sortable: false, align: 'center'},
                            {name: 'cantidad', index: 'cantidad', width: 50, sortable: false, align: 'right'},
                            {name: 'preciounitario', index: 'preciounitario', width: 50, sortable: false, align: 'right'},
                            {name: 'preciototal', index: 'preciototal', width: 50, sortable: false, align: 'right'},
                            {name: 'estadoinventario', index: 'estadoinventario', width: 50, sortable: false, align: 'center', formatter: activoFormatter}
                        ],
                        multiselect: false,
                        paging: true,
                        rowNum: 10,
        ...
        <script type="text/javascript">
            function registraInventario() {
                ...
                var object = {
                    txtalmacenid: txtalmacenid, txtproductoid: txtproductoid, txtcantidad: txtcantidad
                };

                $.ajax({
                    type: "get",
                    url: "registarInventario.htm",
                    data: object,
                    dataType: "json",
