...
<html>
    ...
    <body onload="getData();">
        ...
        <div style="display: none;" id="dialog_add_client" title="Mantenimiento Calibres">
        ...
        <div role="main" id="main" class="container_12 clearfix">
            <section id="content" class="container_12 clearfix" data-sort="true">
                <form action="" class="grid_12">
                    <legend>CALIBRES</legend> 
                    <div class="content">
                        <div class="tabletools">
                            <div class="left">
                                <a class="open-add-client-dialog" href="javascript:void(0);"><i class="icon-plus"></i>Nuevo Calibre</a>
                            </div>                         
                        </div>                        
                    </div>     
                    <table id="list"></table> 
        ...
        <script>
            function getData() {
                $(document).ready(function() {
                    ...
                    $("#list").jqGrid({
                        url: "recuperarCalibres.htm",
                        datatype: 'json',
                        mtype: 'GET',
                        height: 'auto',
                        width: 'auto',
                        colNames: ['NRO', 'DENOMINACION', 'ESTADO', 'EDIT', 'DEL'],
                        colModel: [
                            {name: 'calibresid', index: 'calibresid', width: 50, sortable: false, align: 'left'},
                            {name: 'denominacion', index: 'denominacion', width: 300, sortable: false, align: 'left'},
                            {name: 'estadocalibres', index: 'estadocalibres', width: 100, sortable: false, align: 'center', formatter: activoFormatter},
                            {name: "edit", index: "edit", width: 50, sortable: false, label: " "},
                            {name: "del", index: "del", width: 50, sortable: false, label: " "}
                        ],
                        multiselect: false,
                        paging: true,
                        rowNum: 10,
        ...
        <script type="text/javascript">
            ...
            function Editar(id) {
                ...
                $('#container').data("calibresid", data.calibresid);
                $('#txtdenominacion').val(data.denominacion);
                $('#txtdescripcion').val(data.medida);
                cargarVariedad();
                $('#variedad option[value="' + data.variedadid + '"]').attr('selected', 'selected');
                $("#dialog_add_client").dialog("open");
                return false;
            ...
            function Eliminar(id) {
                var object = {calibresid: id};
                $.ajax({
                    type: "get",
                    url: "deleteCalibres.htm",
                    data: object,
                    dataType: "json",
            ...
            function Update() {
                var object = {
                    calibresid: $('#container').data("calibresid"),
                    variedadid: $('#variedad').val(),
                    denominacion: $('#txtdenominacion').val(),
                    medida: $('#txtMedida').val()
                };
                $.ajax({
                    type: "get",
                    url: "updateCalibres.htm",
                    data: object,
                    dataType: "json",
                    success: function(response) {
                        $("#list").trigger("reloadGrid");
